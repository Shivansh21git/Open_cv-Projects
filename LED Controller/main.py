...
with mp.solutions.hands.Hands(min_detection_confidence=0.1,
               min_tracking_confidence=0.1) as hn:
    while True:
        ret,image=video.read()
        image=cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        image.flags.writeable=False
        tracking= hn.process(image)   
        image.flags.writeable=True
        image=cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        pointList=[]
        if tracking.multi_hand_landmarks:
            for hand_landmark in tracking.multi_hand_landmarks:
                myHands=tracking.multi_hand_landmarks[0]
                for id, lm in enumerate(myHands.landmark):
                    h,w,c=image.shape
                    cx,cy= int(lm.x*w), int(lm.y*h)
                    pointList.append([id,cx,cy])
                mp.solutions.drawing_utils.draw_landmarks(image, hand_landmark, mp.solutions.hands.HAND_CONNECTIONS)
        fingers=[]
        if len(pointList)!=0:
            if pointList[tip[0]][1] > pointList[tip[0]-1][1]: 
                fingers.append(1)
            else:
                fingers.append(0)
            for id in range(1,5):
                if pointList[tip[id]][2] < pointList[tip[id]-2][2]:
                    fingers.append(1)
                else:
                    fingers.append(0)
